...
export const sendRefreshToken = async () => {
    ...
    try {
        response = (await axios.post(
            `${process.env.VUE_APP_IAM_API_URL}/auth/refresh-token`,
            {},
            {
                headers: {
    ...
    if (response?.data?.code === HttpStatus.OK) {
        await appService.setUser(response.data?.data?.user);
        const token: ITokenOption = {
            ...
            accessTokenExpiredAt: +response.data?.data?.accessToken.expiredIn,
            refreshTokenExpiredAt: +response?.data?.data?.refreshToken.expiredIn,
        };
